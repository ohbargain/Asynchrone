...
where Base: Sendable {
    ...
    public init(_ base: Base, selfTerminate: Bool = false) {
        self.base = base
        self.manager = SubSequenceManager<Base>(base, selfTerminate: selfTerminate)
    }
...
extension SharedAsyncSequence {
    ...
    /// Once finished, any calls to yield will result in no change.
    /// - Parameter element: The element to emit.
    public func finish<Element>(with element: Element)
    where Base == PassthroughAsyncSequence<Element> {
...
fileprivate actor SubSequenceManager<Base: AsyncSequence> where Base: Sendable {
    ...
    // Private
    private var base: Base
    private var selfTerminate: Bool
    private var continuations: [String: AsyncThrowingStream<Base.Element, Error>.Continuation] = [:]
    private var subscriptionTask: Task<Void, Never>?

    // MARK: Initialization

    fileprivate init(_ base: Base, selfTerminate: Bool) {
        self.base = base
        self.selfTerminate = selfTerminate
    }
    ...
    /// - Returns: An instance that conforms to `AsyncIteratorProtocol`.
    nonisolated fileprivate func makeAsyncIterator()
        -> ThrowingPassthroughAsyncSequence<Base.Element>.AsyncIterator
    {
    ...
    private func _remove(_ id: String) {
        ...
        if selfTerminate && self.continuations.isEmpty {
            self.subscriptionTask = nil
        }
    }

    private func add(
        id: String, continuation: AsyncThrowingStream<Base.Element, Error>.Continuation
    ) {
    ...
    private func subscribeToBaseSequenceIfNeeded() {
        ...
        let selfTerminate = self.selfTerminate
            ...
            do {
                if selfTerminate {
                    for try await value in base {
                        guard await !self.continuations.isEmpty else {
                            break
                        }
...
extension AsyncSequence {
    /// Creates a shareable async sequence that can be used across multiple tasks.
    public func shared(selfTerminate: Bool = false) -> SharedAsyncSequence<Self>
    where Self: Sendable {
        .init(self, selfTerminate: selfTerminate)
    }
}
